function [e_r_pupil,varargout] = FEBio_run_Iris_Active(x_par_normal,lb,ub,r_p,...
...
%     T_Sphicter        = Sphicter muscle stress as a function of time 
%     
%     r_p               = Pupil radius (mm) 
...
iris.febio_spec.Material.material.solid{1, 1}.Attributes.type='neo-Hookean';
iris.febio_spec.Material.material.solid{1, 1}.E.Text = sprintf('%.20f',E);
iris.febio_spec.Material.material.solid{1, 1}.v.Text = sprintf('%.20f',v);

iris.febio_spec.Material.material.solid{1, 2}.T0.Attributes.type = 'math';

% radius of pupil is 3.4mm and sphincter is set by user
iris.febio_spec.Material.material.solid{1, 2}.T0.Text = sprintf('step(%f+%f-(X^2+Y^2)^.5)',[r_p,a_Sphincter]);
%% Output Logfile
% circle.febio_spec.Output=[];
% circle.febio_spec.Output.Attributes.from='output_format.feb';
%% LoadData
for i=1:size(load,1)
    iris.febio_spec.LoadData.load_controller.points.point{i}.Text ...
...
if strfind(scan_log{end-1},' N O R M A L   T E R M I N A T I O N')
    ...
    detail.rho = sqrt(node_x.^2 + node_y.^2); %the radial distance of each node
        
    dia_pupil = 2*min(detail.rho);% the diameter of the pupil
    
    e_r_pupil = 1/2*((dia_pupil/(2*r_p)).^2-1);%lagrangian strain of the pupil
